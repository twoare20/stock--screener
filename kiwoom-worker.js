...
async function addTradeHistory(env, record) {
  const raw = await env.TRADE_KV.get('trade_history', 'json') || [];
  raw.unshift(record); // 최신순
  const trimmed = raw.slice(0, 200); // 최대 200개
  await env.TRADE_KV.put('trade_history', JSON.stringify(trimmed));
}
...
function isMarketOpen() {
  ...
  const t = h * 60 + m;
  // 모의: 09:00~15:30 / 실전: 08:00~20:00 (NXT포함)
  if (IS_PAPER) return t >= 9 * 60 && t < 15 * 60 + 30;
...
async function fetchCandidates(token) {
  ...
  const [r1, r2] = await Promise.all([
    apiPostRk(token, 'ka10030', { ...params, mrkt_tp: '001' }),  // 코스피
    apiPostRk(token, 'ka10030', { ...params, mrkt_tp: '101' })  // 코스닥
  ]);
  const codes = {};
  ...
  for (const code of codes) {
    ...
    if (result.sell) {
      ...
      if (ok) {
        ...
        await addTradeHistory(env, { type:'sell', code, name:p.name, price:result.curPrice, qty:p.qty, pct:result.pct, reason:result.reason, time:Date.now() });
      }
    } else {
...
async function cronScanAndBuy(env) {
  ...
  const positions = await loadPositions(env);
  const settings = await loadSettings(env);
  const maxH = parseInt(settings.maxHold) || 10;
  ...
  for (const code of codes) {
    ...
    if (matched) {
      ...
      if (pos) {
        ...
        await addTradeHistory(env, { type:'buy', code, name:s.name, price:s.price, qty:pos.qty, time:Date.now(), ordNo:pos.ordNo||'' });
      }
...
export default {
  // Cron 트리거
// ── 일별 요약 저장 ──────────────────────────────────────────────
async function saveDailySummary(env) {
  const now = new Date();
  const kst = new Date(now.getTime() + 9 * 60 * 60 * 1000);
  const dateKey = kst.getUTCFullYear() + '_' +
    String(kst.getUTCMonth()+1).padStart(2,'0') + '_' +
    String(kst.getUTCDate()).padStart(2,'0');
  const histKey = 'trade_history_' + dateKey;
  const summaryKey = 'daily_summary_' + dateKey;

  const history = await env.TRADE_KV.get(histKey, 'json') || [];
  const sells = history.filter(h => h.type === 'sell' && h.pct != null);
  if (!sells.length) return;

  const wins = sells.filter(h => h.pct > 0);
  const losses = sells.filter(h => h.pct <= 0);
  const avgWin = wins.length ? wins.reduce((s,h) => s+h.pct, 0)/wins.length : 0;
  const avgLoss = losses.length ? losses.reduce((s,h) => s+h.pct, 0)/losses.length : 0;
  const profitFactor = avgLoss !== 0 ? Math.abs(avgWin/avgLoss) : null;
  const maxWin = sells.length ? Math.max(...sells.map(h=>h.pct)) : 0;
  const maxLoss = sells.length ? Math.min(...sells.map(h=>h.pct)) : 0;

  // 실현손익 계산
  let totalPl = 0;
  sells.forEach(h => {
    const sellP = h.price || 0;
    const buyP = h.buyPrice || 0;
    if (sellP && buyP) {
      const pl = (sellP - buyP) * h.qty;
      const fee = sellP*h.qty*0.00015 + buyP*h.qty*0.00015;
      const tax = sellP*h.qty*0.002;
      totalPl += pl - fee - tax;
    }
  });
  ...
  await env.TRADE_KV.put(summaryKey, JSON.stringify(summary));
  console.log('일별 요약 저장:', dateKey, summary);
}
  ...
  async scheduled(event, env, ctx) {
    ...
    const min = kstH * 60 + kstM; // KST 분
    const is5min = kstM % 5 === 0;

    // 모의: 15:35 / 실전: 20:00 → 일별 요약 저장
    const isPaper = IS_PAPER;
    const isMarketClose = isPaper ? (kstH === 15 && kstM === 35) : (kstH === 20 && kstM === 0);
    if (isMarketClose) {
      ctx.waitUntil(saveDailySummary(env));
      return;
    }
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/status') {
      const positions = await loadPositions(env);
      const settings = await loadSettings(env);
      const tradeHistory = await env.TRADE_KV.get('trade_history', 'json') || [];
      return new Response(JSON.stringify({ positions, settings, tradeHistory, time: new Date().toISOString() }), {
    ...
    // /history/bulk POST → 히스토리 날짜별 저장
    if (url.pathname === '/history/bulk' && request.method === 'POST') {
      const records = await request.json();
      const kstOffset = 9 * 60 * 60 * 1000;
      const byDate = {};
      records.forEach(h => {
        const ts = h.timestamp || (typeof h.time === 'number' ? h.time : 0);
        if (!ts) return;
        const d = new Date(ts + kstOffset);
        const dateKey = 'trade_history_' + d.getUTCFullYear() + '_' + String(d.getUTCMonth()+1).padStart(2,'0') + '_' + String(d.getUTCDate()).padStart(2,'0');
        if (!byDate[dateKey]) byDate[dateKey] = [];
        byDate[dateKey].push(h);
      });
      await Promise.all(Object.entries(byDate).map(async ([key, arr]) => {
        const existing = await env.TRADE_KV.get(key, 'json') || [];
        const merged = [...arr];
        existing.forEach(h => {
          const exists = merged.some(r => Math.abs((r.timestamp||r.time||0)-(h.timestamp||h.time||0)) < 5000 && r.code===h.code && r.type===h.type);
          if (!exists) merged.push(h);
        });
        await env.TRADE_KV.put(key, JSON.stringify(merged));
      }));
    ...
    // /history POST → 히스토리 추가 날짜별 저장 (앱에서 푸시)
    if (url.pathname === '/history' && request.method === 'POST') {
      const record = await request.json();
      const ts = record.timestamp || (typeof record.time === 'number' ? record.time : 0);
      const kstOffset = 9 * 60 * 60 * 1000;
      const d = new Date((ts||Date.now()) + kstOffset);
      const dateKey = 'trade_history_' + d.getUTCFullYear() + '_' + String(d.getUTCMonth()+1).padStart(2,'0') + '_' + String(d.getUTCDate()).padStart(2,'0');
      const raw = await env.TRADE_KV.get(dateKey, 'json') || [];
      const exists = raw.some(h => Math.abs((h.timestamp||h.time||0)-(ts||0)) < 5000 && h.code===record.code && h.type===record.type);
      if (!exists) {
        raw.unshift(record);
        await env.TRADE_KV.put(dateKey, JSON.stringify(raw));
      }
      return new Response(JSON.stringify({ ok: true }), {
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
      });
