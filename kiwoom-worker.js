...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // /history/bulk POST → 히스토리 전체 업로드
    if (url.pathname === '/history/bulk' && request.method === 'POST') {
      const records = await request.json();
      const existing = await env.TRADE_KV.get('trade_history', 'json') || [];
      ...
      merged.sort(function(a,b){ return (b.timestamp||b.time||0) - (a.timestamp||a.time||0); });
    ...
    // /history POST → 히스토리 추가 (앱에서 푸시)
    if (url.pathname === '/history' && request.method === 'POST') {
      const record = await request.json();
      const raw = await env.TRADE_KV.get('trade_history', 'json') || [];
      // 중복 체크
      const exists = raw.some(h => Math.abs((h.timestamp||h.time||0) - (record.timestamp||record.time||0)) < 5000 && h.code === record.code && h.type === record.type);
      if (!exists) {
        raw.unshift(record);
